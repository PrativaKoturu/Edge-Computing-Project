...
## Where to Show the MQTT Connection

This is one of the most concrete things to point to — it proves the two systems are actually talking to each other over the internet in real time.

---

### 1. The log line when Docker starts — easiest to show
...
Point to these lines at startup:
```
INFO trainer: Connecting to MQTT broker.hivemq.com:1883 as trainer-12345
INFO trainer: MQTT connected rc=0
INFO trainer: Subscribed edge/zone-a/telemetry
INFO trainer: Trainer loop started. Nodes=zone-a
```

**What to say:**
> "`rc=0` means return code 0 — connection accepted. The trainer is now subscribed to `edge/zone-a/telemetry`. Any message the ESP32 publishes to that topic will arrive here."

---

### 2. The code that makes the connection — `control-plane/control_plane/mqtt_client.py`

Open the file and point to **line 31**:
```python
self._client.connect(self._cfg.host, self._cfg.port, keepalive=30)
```
> "This is the TCP connection being opened to `broker.hivemq.com` on port 1883."

Point to **lines 54–56**:
```python
def _on_connect(self, client, userdata, flags, rc):
    self._connected = True
    self._log.info("MQTT connected rc=%s", rc)
```
> "This callback fires the moment the broker confirms the connection. `rc=0` = success. This is where the `MQTT connected` log line comes from."

Point to **lines 45–48**:
```python
...
```
> "After connecting, the trainer immediately subscribes to `edge/zone-a/telemetry`. From this point, every message the ESP32 publishes on that topic gets delivered here and triggers the `on_telemetry` handler in the trainer."

---

### 3. The Wokwi serial monitor — most visual proof

Point to these lines in the ESP32 serial output in the browser:
```
Connecting MQTT broker.hivemq.com:1883 client=edge-zone-a-3f2a1b
MQTT connected. Subscribed: edge/zone-a/request, edge/zone-a/policy
```

The code behind this — `firmware/src/main.cpp` lines 263–278:
```cpp
...
```
> "The ESP32 connects to the same broker. Both the trainer and the ESP32 are now on the same broker — one subscribed to telemetry, the other subscribed to requests. That is how they talk to each other without knowing each other's IP address."

---

### 4. The full connection picture

```
trainer (Docker)                   broker.hivemq.com                ESP32 (Wokwi)
────────────────                   ─────────────────                ─────────────

connects → port 1883               accepts both                     connects → port 1883
subscribes:                        connections                      subscribes:
  edge/zone-a/telemetry  ◄──────── routes messages ────────────►     edge/zone-a/request
                                                                       edge/zone-a/policy
publishes:
  edge/zone-a/policy     ──────────────────────────────────────►   receives policy weights

                          ◄──────────────────────────────────────  publishes:
                                                                       edge/zone-a/telemetry
```

**What to say:**
> "Neither the trainer nor the ESP32 knows the other's IP address. They only know the broker address and the topic names. The broker is the post office — it routes messages between them. This is called the publish-subscribe pattern and it is the standard protocol for all industrial IoT systems."

---

### 5. The three MQTT topics — what flows on each one

| Topic | Direction | What it carries |
|-------|-----------|----------------|
| `edge/zone-a/request` | Docker → ESP32 | Sensor stream request from the dataset: `stream_id`, `payload_kb`, `anomaly` |
| `edge/zone-a/telemetry` | ESP32 → Docker | What the ESP32 did: `cache_hit`, `latency_ms`, `score_int32`, `decision` |
| `edge/zone-a/policy` | Docker → ESP32 | The 8 trained int8 weights, published every 200 critic updates |

> "Three topics, three directions. Requests go down to the edge. Telemetry comes back up to the trainer. Policy updates go back down. That is the entire communication protocol for this system."
